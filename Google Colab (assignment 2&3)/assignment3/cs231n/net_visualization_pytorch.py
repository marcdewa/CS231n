...
def make_fooling_image(X, target_y, model):
    ...
    while(True):
        ...
        correct_class = scores[:,target_y]
        loss = correct_class
        loss.backward()
        grad = X_fooling.grad
        dx_fooling = learning_rate * grad/torch.norm(grad,2)
        with torch.no_grad():
          X_fooling += dx_fooling
        _,predicted_y = torch.max(scores,1)
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    score = model(img)
    reg = l2_reg*(torch.norm(img,2).pow(2))
    loss = score[:,target_y] - reg
    loss.backward()
    dx = learning_rate* img.grad/torch.norm(img.grad,2)
    with torch.no_grad():
      img+=dx
