...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # f1 = (X.dot(W1))+ b1
        # relu_f1 = np.maximum(0,f1)
        # f2 = f1.dot(W2)+b2
        # scores = f2
        layer1,cache1 = affine_relu_forward(X,W1,b1)
        output,cache2 = affine_forward(layer1,W2,b2)
        scores = output
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.use_batchnorm:
              x, cache_norm = batchnorm_forward(x,gamma,beta,bn_params)
        ...
        for i in range(self.num_layers - 2, -1, -1):
            ...
            dx1 = relu_backward(dout,caches[i])
            if self.use_batchnorm:
              dx2, dgamma, dbeta = batchnorm_backward(dx1,caches_norm[i])
              dx3, dw, db = affine_backward(dx2, fc_caches[i])
            else:
              dx3, dw, db = affine_backward(dx1, fc_caches[i])
            if self.use_batchnorm:
